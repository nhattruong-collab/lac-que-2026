...
export const HorseAnimation: React.FC<HorseAnimationProps> = ({ 
  ...
  const [imgError, setImgError] = useState(false); // Chỉ bật lỗi nếu ảnh thực sự không load được

  // Preload hình ảnh (Tải ngầm để chạy cho mượt)
  useEffect(() => {
    const images = [1, 2, 3, 4];
    images.forEach(i => {
      const img = new Image();
      // Sử dụng đường dẫn tương đối (bỏ dấu / ở đầu) để tương thích tốt hơn
      img.src = `horse${i}.png`;
    });
  }, []);

  // Logic chuyển đổi frame cho ảnh PNG
  useEffect(() => {
    let frameInterval: NodeJS.Timeout;
    
    // Chạy animation frame nếu đang chạy và chưa gặp lỗi ảnh nghiêm trọng
  ...
  useEffect(() => {
    if (isRunning && mode === 'run-across') {
      const timer = setTimeout(() => {
        onFinish();
      }, TOTAL_DURATION); 
      return () => clearTimeout(timer);
    }
  }, [isRunning, onFinish, TOTAL_DURATION, mode]);
  ...
  return (
    <div className="w-full h-64 relative overflow-hidden flex items-center bg-transparent group">
      ...
      <style>{`
        ...
        .mode-across { animation: runAcrossLoop ${LAP_DURATION_MS}ms linear ${LAPS}; }
      ...
      {isRunning ? (
        <div className={`horse-wrapper top-16 ${mode === 'run-across' ? 'mode-across' : 'mode-in-place'}`}>
            <div className="horse-body-container">
              {/* SMART RENDER: Thử load ảnh, nếu lỗi thì dùng SVG */}
              {!imgError ? (
                <img 
                    src={`horse${currentFrame}.png`} 
                    alt="Running Horse" 
                    className="w-full h-full object-contain drop-shadow-xl"
                    onError={(e) => {
                        // Thử log ra đường dẫn đầy đủ để debug
                        console.error(`Lỗi tải ảnh: ${e.currentTarget.src}. Chuyển sang SVG.`);
                        setImgError(true);
                    }}
                />
              ) : (
                // SVG Fallback (Con ngựa vector)
            ...
            <div className="dust-particle d3"></div>
             ...
             <div className="absolute -top-16 -right-12 animate-bounce bg-white border-4 border-red-600 rounded-2xl px-4 py-3 shadow-2xl z-20 min-w-[140px] text-center transform scale-110">
           ...
           <div className="w-40 h-40">
              {!imgError ? (
                  ...
                  <svg viewBox="0 0 200 200" className="w-full h-full drop-shadow-md" xmlns="http://www.w3.org/2000/svg">
                      {/* SVG Đứng yên - Copy từ fallback nhưng không có animation leg */}
